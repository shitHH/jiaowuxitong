...
import com.etc.resource.AliPayResource;
import com.etc.service.RecordService;
import com.etc.util.DateUtil;
import com.etc.util.JsonResult;
...
public class AlipayController {
    ...
    Logger log = Logger.getLogger(AlipayController.class);

    /**
     * 前往支付宝进行支付
     */
    @RequestMapping(value="/goAlipay")
    public JsonResult goAlipay(String merchantUserId, String merchantOrderId, String merchantName, String money, HttpSession session) throws Exception{
        ...
        // 付款金额, 必填 单位元
        String total_amount = money;	// 测试用 1分钱
        // 订单名称, 必填
        String subject = merchantName+"-付款用户[" + merchantUserId + "]";
        // 商品描述, 可空, 目前先用订单名称
        String body = subject;
...
//            alipayForm = alipayClient.pageExecute(alipayRequest).getBody();
            alipayForm = alipayClient.pageExecute(alipayRequest,"GET").getBody();
        ...
        StudentVO student = (StudentVO) currentUser;
        Record record = new Record();
        record.setsId(student.getsId());
        record.setmYear("2022");
